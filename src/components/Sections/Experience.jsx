...
const Experience = () => {
  ...
  return (
    <section id="experience" className="relative min-h-screen py-20 flex items-center overflow-visible">
      {/* 100vw Background Panel - Solid in Light, Glass in Dark */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[100vw] h-full bg-slate-50 dark:bg-white/5 border-y border-slate-200 dark:border-white/10 -z-10"></div>
          ...
          <div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 gap-4 lg:gap-8">
                {skills.map((skill) => (
                  <div key={skill.id || skill.name} className="glass p-4 lg:p-6 rounded-3xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/5 hover:border-blue-500/30 dark:hover:border-white/20 transition-all duration-500 flex flex-col items-center group shadow-md dark:shadow-none">
          ...
          <div>
            ...
            {loading ? (
              ...
              <div className="relative space-y-12 pl-4 lg:pl-8">
                {/* Vertical Line */}
                <div className="absolute left-0 top-0 h-full w-[2px] bg-slate-200 dark:bg-white/10"></div>

                {education.map((item, index) => (
                  <div key={item.id || index} className="relative group ml-4 lg:ml-0">
                    {/* Glowing Dot */}
                    <div className="absolute -left-[22px] lg:-left-[42px] top-1 h-3 w-3 lg:h-4 lg:w-4 rounded-full bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)] group-hover:scale-150 transition-transform duration-300 ring-4 ring-white dark:ring-black"></div>
                    
                    <div className="glass p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/5 hover:border-blue-500/30 dark:hover:border-white/20 transition-all duration-500 shadow-md dark:shadow-xl group/card">
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-blue-600 dark:text-blue-400 font-mono text-xs lg:text-sm px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20">{item.year}</span>
                      </div>
                      <h3 className="text-xl lg:text-2xl font-black text-slate-900 dark:text-white group-hover/card:text-blue-500 dark:group-hover/card:text-blue-400 transition-colors">{item.title}</h3>
                      <p className="text-blue-600 dark:text-blue-500/80 text-sm lg:text-base font-bold mt-1 mb-4">{item.institution}</p>
